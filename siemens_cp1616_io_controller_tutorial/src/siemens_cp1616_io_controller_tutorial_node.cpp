/*********************************************************************************************//**
* @file cp1616_io_controller_tutorial_node.cpp
* 
...
int main(int argc, char *argv[])
{
  ...
  ros::Subscriber sub_byte = nh.subscribe("/cp_input_byte_topic", 1, &subByteCallback);
  ros::Subscriber sub_real = nh.subscribe("/cp_input_real_topic", 1, &subRealCallback);
  ros::Subscriber sub_byte_array = nh.subscribe("/cp_input_byte_array_topic", 1, &subByteArrayCallback);
  
  //Create publihsers for topics defined in yaml config file
  ros::Publisher pub_byte = nh.advertise<std_msgs::UInt8MultiArray>("/cp_output_byte_topic", 1);
  ros::Publisher pub_real = nh.advertise<std_msgs::UInt8MultiArray>("/cp_output_real_topic", 1);
  ros::Publisher pub_byte_array = nh.advertise<std_msgs::UInt8MultiArray>("/cp_output_byte_array_topic", 1);
  ...
  while(ros::ok())
  {
    ...
    msg_dim.label = "CP_output_byte";       //assign label as defined in yaml config
    msg_dim.size = 1;                     
    ...
    msg_dim.label = "CP_output_real";       //assign label as defined in yaml config
    msg_dim.size = 4;
    msg.layout.dim.clear();
    msg.layout.dim.push_back(msg_dim);    
    ...
    msg_dim.label = "CP_output_byte_array"; //assign label as defined in yaml config
    msg_dim.size = 8;
    msg.layout.dim.clear();
    msg.layout.dim.push_back(msg_dim);    
