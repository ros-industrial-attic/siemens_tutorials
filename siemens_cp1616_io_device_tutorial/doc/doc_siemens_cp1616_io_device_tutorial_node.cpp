...
int main(int argc, char *argv[])
{
  ...
  ros::Subscriber sub_byte = nh.subscribe("/plc_output_byte_topic",
                                           1, &subByteCallback);
  ros::Subscriber sub_real = nh.subscribe("/plc_output_real_topic",
                                           1, &subRealCallback);
  ros::Subscriber sub_byte_array = nh.subscribe("/plc_output_byte_array_topic",
                                           1, &subByteArrayCallback);
  ros::Subscriber sub_bidirect_byte = nh.subscribe("/plc_bidirect_output_topic",
                                           1, &subBidirectByteCallback);
  
  //Create publihsers for topics defined in yaml config file
  ros::Publisher pub_byte = nh.advertise<std_msgs::UInt8MultiArray>
                                           ("/plc_input_byte_topic", 1);
  ros::Publisher pub_real = nh.advertise<std_msgs::UInt8MultiArray>
                                           ("/plc_input_real_topic", 1);
  ros::Publisher pub_byte_array = nh.advertise<std_msgs::UInt8MultiArray>
                                           ("/plc_input_byte_array_topic", 1);
  ros::Publisher pub_bidirect_byte = nh.advertise<std_msgs::UInt8MultiArray>
                                           ("/plc_bidirect_input_topic", 1);
  ...
  while(ros::ok())
  {
    ...
    msg_dim.label = "PLC_input_byte_array"; 
    msg_dim.size = 8;
    msg.layout.dim.clear();
    msg.layout.dim.push_back(msg_dim);    
